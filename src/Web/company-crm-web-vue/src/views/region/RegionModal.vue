...
function saveItem() {
  ...
  if (item.id > 0) {
    axios.patch('region/' + item.id, item).then((res) => {
      toastr.success('Region updated!', 'Updated')
      emit('onSaved', item)
    })
  } else {
    axios.post('region/', item).then((res) => {
      toastr.success('Region created!', 'Created')
      emit('onSaved', item)
    })
  }
}
      ...
      <div class="modal-content" v-if="item">
        <form @submit.prevent="true">
          <div class="modal-header">
            <h5
              class="modal-title"
              v-text="item.id > 0 ? 'Edit Region' : 'New Region'"
            ></h5>
            <button
              type="button"
              class="btn-close"
          ...
          <div class="modal-body">
            <div class="row mb-3">
              <label class="col-sm-4 col-form-label required">Region Name</label>
              <div class="col-sm-8">
                <input
                  name="name"
                  v-model="item.name"
